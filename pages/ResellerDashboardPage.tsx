...
import { Plus, Home, ShoppingBag, Package } from 'lucide-react';
...
const ResellerDashboardPage: React.FC = () => {
    ...
    const orderColumns = useMemo(() => [
        ...
        { header: 'Status', accessor: 'status', render: (item: Order) => <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800`}>{item.status}</span> },
    ], []);
...
const CreateStoreForm: React.FC<{ onCreate: (data: Omit<Store, 'id'|'ownerId'>) => Promise<void> }> = ({ onCreate }) => {
    ...
    return (
        <>
            <div className="bg-slate-100/80 backdrop-blur-md border border-slate-200 p-8 rounded-lg shadow-md">
                <h2 className="text-2xl font-bold mb-4">Create Your Store</h2>
                <p className="text-slate-600 mb-6">You don't have a store yet. Fill out the form below to get started!</p>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Store Name" required className="w-full p-2 border rounded-md bg-slate-50/50" />
                    <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Store Description" required className="w-full p-2 border rounded-md bg-slate-50/50" />
                    <input type="text" value={logoUrl} onChange={e => setLogoUrl(e.target.value)} placeholder="Logo Image URL" required className="w-full p-2 border rounded-md bg-slate-50/50" />
                    <Button type="submit" className="bg-indigo-600 text-white hover:bg-indigo-700">Create Store</Button>
                </form>
            </div>
            <ConfirmationModal
...
const EditStoreForm: React.FC<{ store: Store, onUpdate: (data: Store) => Promise<void> }> = ({ store, onUpdate }) => {
    ...
    return (
        <div className="bg-slate-100/80 backdrop-blur-md border border-slate-200 p-8 rounded-lg shadow-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <input type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Store Name" required className="w-full p-2 border rounded-md bg-slate-50/50" />
                <textarea name="description" value={formData.description} onChange={handleChange} placeholder="Store Description" required className="w-full p-2 border rounded-md bg-slate-50/50" />
                <input type="text" name="logoUrl" value={formData.logoUrl} onChange={handleChange} placeholder="Logo Image URL" required className="w-full p-2 border rounded-md bg-slate-50/50" />
