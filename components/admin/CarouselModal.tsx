...
interface CarouselModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (slide: any) => Promise<void>;
  defaultValues?: CarouselSlide | null;
  mode: 'add' | 'edit';
}

const CarouselModal: React.FC<CarouselModalProps> = ({ isOpen, onClose, onSubmit, defaultValues, mode }) => {
  const [isLoading, setIsLoading] = useState(false);
  const getInitialState = () => ({
    title: '', subtitle: '', imageUrl: '', buttonText: '', link: ''
  });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`${mode === 'add' ? 'Add' : 'Edit'} Carousel Slide`}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <input type="text" name="subtitle" value={formData.subtitle} onChange={handleChange} placeholder="Subtitle" required className="block w-full p-2 border rounded-md" />
        ...
        <input type="text" name="buttonText" value={formData.buttonText} onChange={handleChange} placeholder="Button Text" required className="block w-full p-2 border rounded-md" />
